...
class ReportsView(BaseView):
    ...
    def create_evolution_chart(self, parent):
        ...
        ctk.CTkLabel(
            header, text="Evolución del Inventario",
            font=ctk.CTkFont(size=16, weight="bold"), text_color="#1E293B"
        ).pack(side='left')

        # Refresh button: recarga datos y redibuja el gráfico
        ctk.CTkButton(
            header, text="🔄", width=36, height=28,
            fg_color="transparent", text_color="#475569",
            hover_color="#F1F5F9", corner_radius=6,
            command=self.refresh_evolution_chart
        ).pack(side='right')
        ...
        # Guardar referencias para poder refrescar el gráfico posteriormente
        try:
            self.evolution_card = chart_card
    ...
    def refresh_evolution_chart(self):
        """Recarga datos y redibuja el gráfico de evolución del inventario."""
        try:
            # Intentar leer fechas de los campos de la UI
            start_date = None
            end_date = None
            try:
            ...
            try:
                ...
                if e:
                    end_date = datetime.strptime(e, "%d/%m/%Y")
            except Exception:
                end_date = None

            # Obtener datos desde el controlador (si vienen fechas, pasar el rango)
            months, values = self.controller.get_inventory_evolution(months=6, start_date=start_date, end_date=end_date)

            # Update stored inventory_evolution so other parts of the UI stay consistent
            try:
                self.inventory_evolution = (months, values)
            except Exception:
                pass

            if not hasattr(self, 'evolution_card') or self.evolution_card is None:
                return

            # Limpiar contenido previo (canvas)
            try:
                if hasattr(self, 'evolution_canvas') and self.evolution_canvas:
                    try:
                        self.evolution_canvas.get_tk_widget().destroy()
                    except Exception:
                        pass
            except Exception:
                pass
            ...
            if not months or not values:
                ctk.CTkLabel(
                    self.evolution_card, text="No hay datos disponibles",
                    font=ctk.CTkFont(size=13), text_color="#94A3B8"
                ).pack(expand=True)
                return
            ...
            fig.patch.set_facecolor('#FFFFFF')
            ax.set_facecolor('#FFFFFF')
            ax.plot(months, values, color='#00B4D8', linewidth=2.5, marker='o', markersize=9,
                    markerfacecolor='#00B4D8', markeredgewidth=0, zorder=3)
            ...
            canvas = FigureCanvasTkAgg(fig, self.evolution_card)
            canvas.draw()
            canvas.get_tk_widget().pack(fill="both", expand=True, padx=15, pady=(0, 15))
            self.evolution_canvas = canvas

            Logger.info("Gráfico de evolución actualizado", "REPORTS_VIEW")

        except Exception as e:
            Logger.error(f"Error refrescando gráfico de evolución: {e}", "REPORTS_VIEW")
